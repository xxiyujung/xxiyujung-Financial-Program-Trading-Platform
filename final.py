...
html_temp = """
		...
		<h1 style="color:blue;text-align:center;">金融看板 </h1>
...
st.subheader("選擇開始與結束的日期, 區間:2022-02-03 至 2022-10-18")
start_date = st.text_input('開始日期 ', '2022-01-03')
end_date = st.text_input('結束日期 , '2022-11-18')
...
KBar_dic['product'] = np.repeat('2330', KBar_dic['open'].size)
...
ShortMAPeriod=st.slider('選擇一個整數', 0, 50, 2)


KBar_df['MAlong'] = KBar_df['close'].rolling(window=LongMAPeriod).mean()
KBar_df['MAshort'] = KBar_df['close'].rolling(window=ShortMAPeriod).mean()
...
LongRSIPeriod=st.slider('選擇一個整數', 0, 500, 10)
